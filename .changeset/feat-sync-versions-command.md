...
Add `mc versions`, a top-level CLI command that synchronizes internal workspace dependency constraints with each package's canonical version. This is intended for migrating existing monorepos to monochange and for keeping internal dependency references consistent after version changes across Cargo, Dart, Deno, Go, npm, and Python workspaces.
...
- **CLI command** `mc versions [--dry-run] [--strategy <default|exact|caret|compatible>] [--format <text|json>]` orchestrates discovery, version planning, JSON/text output, and per-ecosystem file updates.
...
Currently supports **Cargo**, **Dart**, **Deno**, **Go**, **npm**, and **Python** manifests for internal workspace dependency synchronization.
