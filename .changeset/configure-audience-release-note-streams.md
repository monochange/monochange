...
> **Breaking change:** callers that construct the public raw changelog configuration structs with literals must initialize the new stream and output fields. TOML users remain backward compatible because omitted routes resolve to the built-in `default` stream.
