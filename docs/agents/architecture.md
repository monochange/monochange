...
- `crates/monochange` stays a tiny published binary/facade crate. `crates/monochange_cli` orchestrates commands by dispatching to adapters and should not own adapter implementation details.
...
2. Did `crates/monochange_cli` add adapter-specific mutation logic instead of dispatching to an adapter?
...
- Provider dispatch in `crates/monochange_cli` should stay concentrated in small orchestration modules such as `src/hosted_sources.rs`, `src/release_artifacts.rs`, and narrowly-scoped release helpers until the behavior can move behind adapter contracts.
- Ecosystem dispatch in `crates/monochange_cli` should stay concentrated in shared orchestration helpers such as `src/versioned_files.rs` until the behavior can move behind adapter contracts.
