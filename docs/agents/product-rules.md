...
- Only `.monochange/local/` may be gitignored; never ignore all of `.monochange/`. Release records (`.monochange/releases/<id>/release.json`) and prerelease state are committed release state that `CommitRelease`, `publish-readiness`, `tag-release`, and provider release automation read from git history, so ignoring the directory makes releases unpublishable.
