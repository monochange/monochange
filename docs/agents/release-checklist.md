...
- [ ] Run `monochange step placeholder-publish` to publish placeholder versions for every missing package. This step is manual and must happen **before** the release PR is merged.
...
- [ ] Run `monochange step validate` and confirm no warnings about group members that contradict their package manifests.
...
- [ ] Run `monochange step plan-publish-rate-limits --mode publish --format json` locally and inspect the output. Confirm that:
  ...
  - npm platform packages are **excluded** from the `monochange step plan-publish-rate-limits` output if they are no longer published by this script (npm publishing is now handled by `monochange step publish-packages`).
...
npm packages are handled differently from cargo crates. Platform-specific npm packages (`@monochange/cli-darwin-arm64`, etc.) and the main CLI wrapper (`@monochange/cli`) are built and populated by `scripts/npm/populate-packages.ts` inside the `publish.yml` **plan** job. They are now published by `monochange step publish-packages` alongside cargo crates in the **publish** job.
...
- [ ] Verify that `monochange step plan-publish-rate-limits` does **not** re-include npm packages in its batch output for a second publish attempt. If `monochange step plan-publish-rate-limits` cannot filter by ecosystem, pass `--package` flags to exclude npm packages from the cargo publish.
...
- [ ] Confirm `monochange step release-record --from HEAD` shows the release record as resolved.
