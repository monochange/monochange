...
npm packages are handled differently from cargo crates. Platform-specific npm packages (`@monochange/cli-darwin-arm64`, etc.) and the main CLI wrapper (`@monochange/cli`) are built and populated by `scripts/npm/populate-packages.ts` inside the `publish.yml` **plan** job. They are now published by `mc step:publish-packages` alongside cargo crates in the **publish** job.
