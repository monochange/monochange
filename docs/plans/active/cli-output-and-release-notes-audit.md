...
- [ ] Add binary-level integration tests that capture stdout, stderr, and exit status.
- [ ] Assert that `check` exits with the same status for text, Markdown, JSON, and compact JSON.
- [ ] Fix the JSON branch in `run_check_command` to fail on `lint_has_errors`.
- [ ] Add tests for `NO_COLOR`, `MONOCHANGE_NO_PROGRESS`, `TERM=dumb`, a common CI variable, and non-TTY stderr.
- [ ] Stop both existing progress reporters from writing cursor controls outside an interactive renderer.
- [ ] Store and honor `enabled` in `HumanLintProgressReporter`.
