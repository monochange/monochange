...
- Durability: persisted by `monochange step publish-readiness --output <path>` and consumed by publish planning
...
- Durability: persisted by `monochange step placeholder-publish --output <path>` for CI/manual retry/audit notes
...
- Current header: none
- Current path: `monochange step publish-packages --output <path>` / `monochange step publish-packages --resume <path>` flow
