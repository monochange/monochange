...
- **config-driven workflow commands**: every `[cli.<command>]` table in `monochange.toml` becomes `monochange <command>`. `monochange init` does not seed default workflow aliases; add these tables when you want a named workflow that chains steps, adds custom inputs, or runs `Command` steps.
