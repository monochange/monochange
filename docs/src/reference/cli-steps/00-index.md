...
`PrepareRelease` and `DisplayVersions` steps also report per-phase timings when they compute release state. Those timings power the benchmark phase-budget checks for release workflows such as `mc step:prepare-release --dry-run` and configured `mc release` wrappers.
