...
- **immutable built-in step commands**: every built-in step except `Command` is exposed directly as `monochange step <kebab-step-name>`, for example `monochange step discover`, `monochange step prepare-release`, and `monochange step affected-packages`. These commands are generated by the binary, derive their flags from the step schema, and do not require a `[cli.*]` entry in `monochange.toml`.
- **config-driven workflow commands**: every `[cli.<command>]` table in `monochange.toml` becomes `monochange run <command>`. `monochange init` does not seed default workflow aliases; add these tables when you want a named workflow that chains steps, adds custom inputs, or runs `Command` steps.
...
- [VerifyReleaseBranch](17-verify-release-branch.md)
