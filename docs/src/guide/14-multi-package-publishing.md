...
- mixed workspaces where one package needs registry-native steps that do not match `monochange step publish-packages`
...
5. run `monochange step publish-packages --dry-run` after registry enrollment changes
6. optionally run `monochange step publish-readiness --from HEAD --output <path>` as a preflight before real `monochange step publish-packages`
