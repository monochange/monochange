...
Readiness also verifies trusted publishing for every selected package before anything is published:

- **Project side**: the GitHub trust context (`publish.trusted_publishing.repository`, `workflow`, and the optional `environment`) must resolve, the referenced workflow file must exist under `.github/workflows/`, and the current environment must be able to verify the CI/OIDC identity. Failures block the package. Outside a supported CI provider (for example during local development) the finding degrades to `manual_verification_required` instead of blocking, because real publish runs verify the identity at publish time.
- **Registry side**: npm, crates.io, and pub.dev only accept trusted publishing for packages that already exist on the registry, so readiness probes the public registry APIs and blocks packages that have never been published with guidance to bootstrap via `monochange step placeholder-publish`. Existing packages are reported as `manual_verification_required` because trusted publisher entries cannot be read back without registry credentials; the report includes the registry setup URL. Network lookup failures stay non-blocking so readiness remains usable offline.

The report also records the dependency-corrected `publish_order` and validates it against the workspace dependency graph (including dev-dependencies). A package scheduled before one of its workspace dependencies is a blocking finding; a release record whose recorded publication order differs from the corrected plan is reported as a non-blocking note.
