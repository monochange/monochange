...
///
/// This is the library entry point behind `monochange step affected-packages` and the
...
fn render_changeset_verification_comment(
	...
	lines.push(
		"- for example: `monochange change --package <id> --bump patch --reason \"describe the change\"`"
			.to_string(),
	);
	if !verify.skip_labels.is_empty() {
		lines.push(
